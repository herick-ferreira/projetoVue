<template>
  <img alt="Vue logo" src="./assets/logo.png">
  <HelloWorld msg="Welcome to Your Vue.js App" visibilidade="false"/>
  <NovoComponente texto="A diretiva v-text permite a apresentação de textos, tal como ocorre quando usamos diretivas diretas"/>
</template>

<script>
import HelloWorld from './components/HelloWorld.vue'
import NovoComponente from './components/NovoComponente'

export default {
  name: 'App',
  components: {
    HelloWorld,
    NovoComponente
  }
}
</script>

<style>
#app {
  font-family: Avenir, Helvetica, Arial, sans-serif;
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
  text-align: center;
  color: #2c3e50;font-size: xx-large;
  margin-top: 60px;background: linear-gradient(45deg, blue, rgb(93, 2, 2));
}
</style>
